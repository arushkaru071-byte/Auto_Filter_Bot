...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton ('𖦹 Sᴏᴜʀᴄᴇ', callback_data='source'),
        ],[
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ⇋', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons3)
    ...
    try:
        if not spoll:
            ...
            if len(message.text) < 100:
                ...
                stick_id = "CAACAgUAAxkBAAEP05ZpIGPXoLxtwSn0NVpTVLERV6KqZAACgSEAAvHEAAFVdSJabD7OKOw2BA"
...
async def advantage_spell_chok(client, message):
    ...
    d = await message.reply_text(text=script.CUDNT_FND.format(message.from_user.mention), reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=message.id)
    await asyncio.sleep(60)
    await d.delete()
    try:
        await message.delete()
